...
const data = ref({ 
  users: [], 
  files: [], 
  disk_info: { total_gb: 0, used_gb: 0, free_gb: 0, percent_full: 0 },
  file_types: {}
})
...
const deleteUser = async (email) => {
  if(!confirm(`⚠️ DELETE USER ${email}?\nThis will remove their files too.`)) return
...
const deleteFile = async (id) => {
  if(!confirm("⚠️ Delete this file permanently from disk?")) return
    ...
    <aside class="sidebar">
      ...
      <nav class="nav-menu">
        ...
        <button :class="{ active: activeTab === 'users' }" @click="activeTab = 'users'">
          <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle></svg>
          User Directory
        </button>
      ...
      <div class="analytics-row">
        ...
        <div class="stat-card">
          <div class="stat-header">
            ...
            <span class="stat-value">{{ data.disk_info.percent_full }}%</span>
          </div>
          <div class="progress-track">
            <div class="progress-fill" :style="{ width: data.disk_info.percent_full + '%', background: diskColor }"></div>
          </div>
          <div class="stat-meta">
        ...
        <div class="stat-card">
          ...
          <div class="tags-container">
            <span v-for="(count, type) in data.file_types" :key="type" class="type-tag">
              {{ type.toUpperCase() }}: {{ count }}
            </span>
            <span v-if="Object.keys(data.file_types).length === 0" class="type-tag">None</span>
          </div>
        </div>
      </div>
              ...
              <tr v-for="u in data.users" :key="u.email">
                ...
                <td class="col-actions">
                  <div class="action-group">
                    ...
                    <button v-if="u.status === 'ACTIVE'" @click="updateUserStatus(u.email, 'BAN')" class="btn-xs btn-ban">Ban</button>
                    <button v-if="u.status === 'BANNED'" @click="updateUserStatus(u.email, 'APPROVE')" class="btn-xs btn-neutral">Unban</button>
              ...
              <tr v-for="f in data.files" :key="f.id">
                <td class="col-main">
                  <div class="file-cell">
                    ...
                    <span class="file-id mono-num">ID: {{ f.id.substring(0,8) }}...</span>
                  </div>
                </td>
                ...
                <td class="mono-num">{{ f.downloads }}</td>
                <td class="col-actions">
                  <button @click="deleteFile(f.id)" class="btn-xs btn-delete">Delete</button>
                </td>
              </tr>
            </tbody>
          </table>
...
.sidebar { width: 260px; background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(12px); border-right: 1px solid rgba(255,255,255,0.08); display: flex; flex-direction: column; padding: 24px; z-index: 10; }
...
.view-header { display: flex; justify-content: space-between; align-items: flex-end; padding: 32px 40px 20px; background: linear-gradient(180deg, rgba(15,23,42,0.8) 0%, transparent 100%); z-index: 10; }
...
.stat-title { font-size: 0.75rem; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600; }
.stat-value { font-size: 1.1rem; font-weight: 700; color: white; font-family: 'JetBrains Mono', monospace; }
.stat-meta { font-size: 0.75rem; color: #64748b; margin-top: 4px; }
...
.type-tag { font-size: 0.7rem; background: rgba(99,102,241,0.1); color: #818cf8; padding: 4px 8px; border-radius: 4px; border: 1px solid rgba(99,102,241,0.2); font-family: 'JetBrains Mono'; }
